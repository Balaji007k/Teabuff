...
function Menu({ isAuthenticated }) {
  ...
  productsItem?.map((item) => (
    <ProductCard key={item._id} isAuthenticated={isAuthenticated} product={item} Navigate={Navigate} />
  ))
) : filteredProducts.length > 0 ? (
  // Filtered results
  filteredProducts.map((product) => (
    <ProductCard key={product._id} isAuthenticated={isAuthenticated} product={product} Navigate={Navigate} />
  ))
) : (
  // No results
  <EmptyProductCard />
