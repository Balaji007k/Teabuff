...
export default function ProductFilters({ Products, id, searchingProduct }) {
  ...
  const handleCategorySelect = (categoryId) => {
    ...
    if (categoryId !== 0) {
      ...
      // ✅ If no products match → show EmptyProductCard
      ...
      // ✅ Cancel category filter → restore full list
      if (Location.pathname === "/Menu") Products(productsItem);
      else Products(null);
  ...
  const handleSort = (value) => {
    ...
    if (value === "BestSelling") {
      ...
      Sorted = [...source].filter(
        (product) => Number(product.rating) >= 4 && Number(product.rating) <= 5
      );
    } else if (value === "LowRating") {
      Sorted = [...source].filter(
        (product) => Number(product.rating) >= 1 && Number(product.rating) < 4
      );
    } else if (value === "Cancel") {
      setSort("");
    ...
    // ✅ Empty state check for sorting
    if (Sorted.length === 0) Products([]);
    else Products(Sorted);
  ...
  // 🔹 Reset filters when id or products change
  useEffect(() => {
    setFilteredCategory(null);
    setSearchItem("");
    setSelectedCategory(0);
    ...
    <div className="w-100 mb-md-4">
      <div className="Cart-head d-flex align-items-center gap-2 ps-2">
        ...
        <div>
          <div className="search justify-content-end">
            <div
              className={`top d-flex align-items-center w-auto ${
