...
const orderData = {
  ...
  newOrder: {
    ...
    subtotal: (cart?.items || []).reduce((acc, item) => acc + (item?.itemPrice || 0) * (item?.quantity || 0), 0),
    shipping: 10,
    tax: ((cart?.items || []).reduce((acc, item) => acc + (item?.itemPrice || 0) * (item?.quantity || 0), 0)) * 0.18,
    total: ((cart?.items || []).reduce((acc, item) => acc + (item?.itemPrice || 0) * (item?.quantity || 0), 0)) + 10 + (((cart?.items || []).reduce((acc, item) => acc + (item?.itemPrice || 0) * (item?.quantity || 0), 0)) * 0.18),
    paymentType: "Visa",
    cardEnding: "5678",
  },
};
